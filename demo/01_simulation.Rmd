...
for(es in c(0,0.5,1,2,3))
{
  ...
  write.table(bedSimMerged[,c("chrom", "chromStart", "chromEnd", "name", "score", "strand"), with=FALSE], filePath, 
  ...
  lapply(names(bedSim),function(sample){
    ...
    write.table(bedSim[[sample]][,c("seqnames", "start", "end", "name", "score", "strand"), with=FALSE], filePath, 
                quote=FALSE, col.names=FALSE,
                sep="\t", 
                row.names=FALSE)})
}
