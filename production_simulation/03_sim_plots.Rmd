...
Paths
```{r, path definition}
haploParaPath <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/lfc_dist/happloinsufficiency.rds"
actParaPath <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/lfc_dist/activation.rds"

esSimDir <- gcVariedSimDir <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/out_final"
gcNonVariedSimDir <- esPosSimDir <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/out_test"
resPath <- "BMresults/bm_res.rds"
atacPeakPath <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/atac_peaks/filtered_mergedSamples_atac_peaks.bed_peaks.narrowPeak"
chipPeaksDir <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/tf_peaks"
motifMatchesPath <- "/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/atac_peaks/pmoi.rds"
...
```{r}
lfcA <- subset(lfcDist, lfcDist$paradigm=="Activation")
lfcA$enr <- scale(lfcA$enr, center=TRUE, scale=TRUE)

ggplot(lfcA, aes(x=enr,y=lfc))+
  geom_density2d_filled()+
  geom_hline(yintercept=0, lty=2, color="white", lwd=1.2)+
  ggtitle("Observed Activation")+
...
ggplot(lfcA, aes(x=enr,y=lfc))+
  ...
  ggtitle("")+
  xlab("Relative enrichment")+
  ylab("Log fold changes")+
  xlim(c(0,100))+
  ylim(c(-2,2))+
  theme_minimal()+
  theme(legend.position = "none")
ggsave("ActFCEnr.png", width=8, height=10, unit="cm")
```

```{r}
lfcH <- subset(lfcDist, lfcDist$paradigm=="Haploinsufficiency")
lfcH$enr <- scale(lfcH$enr, center=TRUE, scale=TRUE)

ggplot(lfcH, aes(x=enr,y=lfc))+
  geom_density2d_filled()+
  geom_hline(yintercept=0, lty=2, color="white", lwd=1.2)+
  ggtitle("")+
  xlab("Relative Enrichment")+
...
ggplot(lfcH, aes(x=enr,y=lfc))+
  ...
  theme_minimal()+
  theme(legend.position = "none")
ggsave("HapFCEnr.png", width=8, height=10, unit="cm")
```
...
allRanks <- lapply(fullResPaths, function(path){
  ...
  if(file.exists(path))
  {
    name <- unlist(tstrsplit(path, split="/", keep=11))
    res <- readRDS(path)
...
#geom_point(aes(size =isMinTop5Es))+
       ...
       guides(color=guide_legend(title=""))+
       theme_minimal()+
        theme(legend.spacing.x = unit(0.5, "cm"),  # Adjust this value as needed
...
gcBiasMethodsPlot
ggsave("gcBiasMethodsPlot.png", gcBiasMethodsPlot, width=25, height=20, unit="cm")
```
...
happControlPlot <- happControlPlot +
  labs(fill = "Rank")

posControlAllPlot <- ggarrange(actControlPlot, happControlPlot, nrow=2, common.legend=TRUE)
posControlAllPlot
ggsave("posControlPlot.png", posControlAllPlot, width=18, height=20, unit="cm")
...
actControlMethodPlot <- actControlMethodPlot +
  labs(fill = "Rank")
...
happControlMethodPlot <- happControlMethodPlot +
  labs(fill = "Rank")

posControlMethodPlot <- ggarrange(actControlMethodPlot, happControlMethodPlot, 
                                  nrow=2, common.legend=TRUE)
posControlMethodPlot
ggsave("posControlMethodPlot.png", posControlMethodPlot, width=18, height=20, unit="cm")
```
...
fracs <- lapply(1:length(namesPeaks), function(i){
  ...
  print(length(subPeaksChIP))
   ...
   fwrite(as.data.table(subPeaksChIP)[,c("seqnames", "start", "end", paste0("V", 4:10))], file.path("/mnt/plger/esonder/R/tf_activity_benchmark/simulations/simulations/production_simulation/tf_peaks", paste(tfName, "peaks_with_motif.bed", sep="_")))
...
a <- ggplot(enrDt, aes(fill=enr_motif_chip_ov_atac, x=tf, y="Ratio"))+
  geom_tile()+
  scale_fill_viridis_c()+
  theme_minimal()+
  ylab("")+
  guides(fill=guide_legend(title="Count(motifs within a ChIP-peak) / Count(motifs outside)",
                           title.position="top"))+
...
b <- ggplot(enrDt, aes(fill=enr_motif_chip_peaks_ov_atac, x=tf, y="Ratio"))+
  geom_tile()+
  scale_fill_viridis_c()+
  theme_minimal()+
  ylab("")+
...
ggsave("Ratio.png", a, width=12, height = 5, units="cm")
```
